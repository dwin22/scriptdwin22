...
[[ "$apache" != "" ]] && s4="apache"
echo -e "${cor[5]} $(fun_trans "Esse e o Fail2ban Protection")"
echo -e "${cor[5]} $(fun_trans "Otimo para proteger contra ataques de DDos")"
echo -e "$barra"
if [[ ! -z "$pid_fail" ]]; then
 echo -e "${cor[2]} [1] >${cor[5]} $(fun_trans "Desinstalar Fail2ban")"
 echo -e "${cor[2]} [2] >${cor[5]} $(fun_trans "Olhar o log")"
 echo -e "${barra}"
  while [[ -z ${logxyz} || ${logxyz} != @(1|2) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Selecione a Opcao"): " && read logxyz
   tput cuu1 && tput dl1
  done
 case ${logxyz} in
...
echo -e "${cor[5]} $(fun_trans "Deseja Instalar o Fail2ban?")"
echo -e "$barra"
  while [[ -z ${fail2ban} || ${fail2ban} != @(s|S|n|N|y|Y) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Selecione a Opcao") [S/N]: " && read fail2ban
   tput cuu1 && tput dl1
  done
if [[ "$fail2ban" = @(s|S|y|Y) ]]; then
apt-get install fail2ban -y &> /dev/null
wget -O $HOME/fail2ban https://www.dropbox.com/s/sq4q2ug1mnbdpw5/fail2ban-0.9.4.tar.gz?dl=0 &> /dev/null
...
echo -e "${cor[5]} $(fun_trans "Fail2ban sera ativo nas Seguintes Portas e Servicos"):"
echo -e "${barra}"
[ "$s1" != "" ] && echo -ne " $s1"
[ "$s2" != "" ] && echo -ne " $s2"
[ "$s3" != "" ] && echo -ne " $s3"
